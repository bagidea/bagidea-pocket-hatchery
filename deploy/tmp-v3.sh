...
rm -rf /tmp/phv3 && mkdir -p /tmp/phv3
cp build/pockethatch.wasm /tmp/phv3/
cp build/pockethatch.abi /tmp/phv3/

echo ""
echo "=== Deploy ==="
$CLEOS set contract pockethatch1 /tmp/phv3 pockethatch.wasm pockethatch.abi \
  -p pockethatch1@active --sign-with "$PH_PUB" 2>&1

echo ""
echo "=== Clear old config (raw DB erase, no deserialization) ==="
