...
echo "=== Compiling burncreature fix ==="
"${CDT_BIN}" \
  --contract pockethatch \
  -I "${CDT_INC_CONTRACTS}" \
  -I "${CDT_INC_CORE}" \
  -I "${SRC_DIR}" \
  -o "${BUILD_DIR}/pockethatch.burnfix.wasm" \
  "${SRC_DIR}/pockethatch.cpp" \
  --abigen

echo ""
echo "=== Build outputs ==="
ls -la "${BUILD_DIR}/pockethatch.burnfix.wasm" "${BUILD_DIR}/pockethatch.burnfix.abi"
sha256sum "${BUILD_DIR}/pockethatch.burnfix.wasm"
echo "Compile OK"
