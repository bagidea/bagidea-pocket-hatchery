...
const URL = 'http://127.0.0.1:4173/plugin/pocket-hatchery/static/?demo'
const OUT = 'hittest-odds.png'
...
// Read the actually-rendered odds chips (not computed — real DOM innerText).
...
// The flat hatch cost shown on the CTA button.
const hatchBtn = await page.$eval('button', (b) => b.innerText.replace(/\s+/g, ' ').trim()).catch(() => null)
...
// The free-to-start path line.
const freePath = await page.$eval('[data-testid="free-path"]', (el) => el.innerText.replace(/\s+/g, ' ').trim()).catch(() => null)

// Is the Rare chip visibly the richest (has a box-shadow glow)? real computed style.
...
// Assertions — fail loud so a broken render can't pass silently.
const fail = []
...
if (!freePath || !freePath.includes('200 EGG') || !freePath.includes('claim HATCH')) fail.push('free-path line missing/incomplete')
